...
@section('naziv', 'Šifarnici | Tipovi štampača')
...
<h1 class="page-header">
    <img class="slicica_animirana" alt="Tipovi stampaca"
         src="{{ url('/images/stampac.png') }}" style="height:64px;">
    &emsp;Tipovi štampača
</h1>
@endsection
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('tipovi_stampaca.izmena') }}" method="post">
                    {{ csrf_field() }}
...
@section('traka')
<h4>Dodavanje novog tipa štampača</h4>
<hr>
<div class="well">
    <form action="{{ route('tipovi_stampaca.dodavanje') }}" method="POST" data-parsley-validate>
        {{ csrf_field() }}
        <div class="form-group{{ $errors->has('naziv') ? ' has-error' : '' }}">
            <label for="naziv">Naziv tipa štampača:</label>
        ...
        <div class="row dugmici">
            <div class="col-md-12" style="margin-top: 20px;">
                <div class="form-group">
                    ...
                    <div class="col-md-6">
                        <a class="btn btn-danger btn-block ono" href="{{route('tipovi_stampaca')}}">
                            <i class="fa fa-ban"></i>&emsp;Otkaži
                        </a>
                    </div>
                </div>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.otvori-brisanje', function () {
            var id = $(this).val();
            $('#idBrisanje').val(id);
            var ruta = "{{ route('tipovi_stampaca.brisanje') }}";
            $('#brisanje-forma').attr('action', ruta);
        });

        $(document).on('click', '.otvori-izmenu', function () {
            var id = $(this).val(                            );
            var ruta = "{{ route('tipovi_stampaca.detalj') }}";
            $.ajax({
                url: ruta,
                type: "POST",
                data: {
