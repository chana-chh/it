<form action="{{ route('stavke.ram.dodavanje') }}" method="POST" data-parsley-validate>
    {{ csrf_field() }}
    ...
    <div class="col-md-12">
        <div class="form-group{{ $errors->has('memorija_model_id') ? ' has-error' : '' }}">
            <label for="memorija_model_id">Model memorije:</label>
            <select name="memorija_model_id" id="memorija_model_id" class="chosen-select form-control"
                    data-placeholder="Model memorije ..." required>
                <option value=""></option>
