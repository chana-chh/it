...
@section('naziv', 'Oprema | Izmena osnovnih podataka računara')
...
<div class="row ceo_dva">
    <div class="col-md-12 boxic">
        <form action="{{ route('racunari.oprema.izmena.post', $uredjaj->id) }}" method="POST" data-parsley-validate>
            {{ csrf_field() }}
            {{-- Red I --}}
            <div class="row">
                <div class="col-md-2">
                    <div class="form-group checkboxoviforme">
                        <label>
                            <input type="checkbox" name="laptop" id="laptop" {!!$uredjaj->laptop == 0 ? "" :'checked="checked"'!!}> &emsp;Da li se radi o laptop računaru?
                        </label>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group checkboxoviforme">
                        <label>
                            <input type="checkbox" name="brend" id="brend" {!!$uredjaj->brend == 0 ? "" :'checked="checked"'!!}> &emsp;Da li se radi o brend računaru?
                        </label>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group checkboxoviforme">
                        <label>
                            <input type="checkbox" name="serverf" id="server" {!!$uredjaj->server == 0 ? "" :'checked="checked"'!!}> &emsp;Da li se radi o serveru?
                        </label>
                    </div>
                </div>
                    ...
                    <div class="form-group{{ $errors->has('proizvodjac_id') ? ' has-error' : '' }}">
                        ...
                        <select name="proizvodjac_id" id="proizvodjac_id" class="chosen-select form-control" data-placeholder="proizvođač ...">
                            ...
                            <option value="{{ $proizvodjac->id }}" 
                                {{ old( 'proizvodjac_id') == $proizvodjac->id ? ' selected' : '' }}
                                {{ $uredjaj->proizvodjac_id == $proizvodjac->id ? ' selected' : '' }}> 
                                {{ $proizvodjac->naziv }}
                        </option>
                        @endforeach
                    </select>
        ...
        <div class="row">
            ...
            <div class="col-md-3 nevidljivi">
                <div class="form-group{{ $errors->has('serijski_broj') ? ' has-error' : '' }}">
                    ...
                    <input type="text" name="serijski_broj" id="serijski_broj" class="form-control" value="{{ old('serijski_broj', $uredjaj->serijski_broj) }}" maxlength="50"> @if ($errors->has('serijski_broj'))
                    <span class="help-block">
            ...
            <div class="col-md-3">
                <div class="form-group{{ $errors->has('inventarski_broj') ? ' has-error' : '' }}">
                    ...
                    <input type="text" name="inventarski_broj" id="inventarski_broj" class="form-control" value="{{ old('inventarski_broj', $uredjaj->inventarski_broj) }}" maxlength="50"> @if ($errors->has('inventarski_broj'))
                    <span class="help-block">
                ...
                <div class="form-group{{ $errors->has('stavka_nabavke_id') ? ' has-error' : '' }}">
                    ...
                    <select name="stavka_nabavke_id" id="stavka_nabavke_id" class="chosen-select form-control" data-placeholder="nabavke ..." required>
                        ...
                        <optgroup label="{{ $o->dobavljac->naziv }}, {{ $o->broj }} od {{ $o->datum }}">
                            ...
                            <option value="{{ $s->id }}" 
                                {{ old( 'stavka_nabavke_id') == $s->id ? ' selected' : '' }}
                                {{ $uredjaj->stavka_nabavke_id == $s->id ? ' selected' : '' }}> 
                                {{$s->naziv}} (popunjeno je {{$s->racunari->count() }} od {{$s->kolicina}}
                                    {{$s->jedinica_mere}})
                        </option>
                        @endforeach
                    </optgroup>
            ...
            <div class="form-group{{ $errors->has('kancelarija_id') ? ' has-error' : '' }}">
                ...
                <select name="kancelarija_id" id="kancelarija_id" class="chosen-select form-control" data-placeholder="kancelarija ...">
                    ...
                    <option value="{{ $kancelarija->id }}" 
                        ...
                        {{ $uredjaj->kancelarija_id == $kancelarija->id ? ' selected' : '' }}>
...
<div class="row">
    ...
    <div class="col-md-3">
        <div class="form-group{{ $errors->has('ime') ? ' has-error' : '' }}">
            ...
            <input type="text" name="ime" id="ime" class="form-control" value="{{ old('ime', $uredjaj->ime) }}" maxlength="100" required> @if ($errors->has('ime'))
            <span class="help-block">
    ...
    <div class="col-md-3">
        <div class="form-group{{ $errors->has('erc_broj') ? ' has-error' : '' }}">
            ...
            <input type="text" name="erc_broj" id="erc_broj" class="form-control" value="{{ old('erc_broj', $uredjaj->erc_broj) }}" maxlength="100" required> @if ($errors->has('erc_broj'))
            <span class="help-block">
...
<div class="col-md-4 nevidljivi">
    <div class="form-group{{ $errors->has('link') ? ' has-error' : '' }}">
        ...
        <input type="url" name="link" id="link" class="form-control" value="{{ old('link', $uredjaj->link) }}" maxlenght="255"> @if ($errors->has('link'))
        <span class="help-block">
...
<script>
    $(document).ready(function () {
        ...
        if (($("#laptop").is(":checked")) || ($("#brend").is(":checked")) || ($("#server").is(":checked"))) {
                $(".nevidljivi").show(300);
                resizeChosen();
            } else {
                $(".nevidljivi").hide();
