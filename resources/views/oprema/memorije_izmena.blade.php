...
@section('naziv', 'Oprema | Izmena memorijskog modula')
...
<div class="row">
    <div class="col-md-8">
        <h1>
            <img class="slicica_animirana" alt="Izmena memorijskog modula"
                 src="{{url('/images/memorija.png')}}" style="height:64px;">
            &emsp;Izmena podataka memorijskog modula
        </h1>
    </div>
</div>
<hr>
...
<div class="row ceo_dva">
    <div class="col-md-12 boxic">
        <form action="{{ route('memorije.oprema.izmena.post', $uredjaj->id) }}" method="POST" data-parsley-validate>
            {{ csrf_field() }}
                        ...
                        <select name="stavka_otpremnice_id" id="stavka_otpremnice_id" class="chosen-select form-control" data-placeholder="otpremnice ..." >
                            ...
                            <optgroup label="{{ $o->dobavljac->naziv }}, {{ $o->broj }} od {{ $o->datum }}">
                                ...
                                <option value="{{ $s->id }}"
                                    ...
                                    {{$s->naziv}} (popunjeno je {{$s->memorije->count() }} od {{$s->kolicina}} {{$s->jedinica_mere}})
                            </option>
                            @endforeach
                        </optgroup>
                        @endforeach
            ...
            <div class="col-md-4">
                <div class="form-group{{ $errors->has('memorija_model_id') ? ' has-error' : '' }}">
                    <label for="memorija_model_id">Model memorijskog modula:</label>
                    <select name="memorija_model_id" id="memorija_model_id" class="chosen-select form-control" data-placeholder="model ..." required>
                        <option value=""></option>
                        @foreach($modeli as $m)
                        <option value="{{ $m->id }}"
                            {{ $m->id == old('memorija_model_id') ? ' selected' : '' }}
                            {{ $uredjaj->memorija_model_id == $m->id ? ' selected' : '' }}>
                            {{ $m->proizvodjac->naziv }}, {{ $m->naziv }} na {{ $m->brzina }} MHz sa {{$m->kapacitet}} MB
                    </option>
                    @endforeach
                </select>
        ...
        <div class="col-md-6">
            <div class="form-group{{ $errors->has('racunar_id') ? ' has-error' : '' }}">
                ...
                <select name="racunar_id" id="racunar_id" class="chosen-select form-control" data-placeholder="računar ..." >
                    ...
                    <option data-procesor="{{ $r->memorije()->count() > 0 ? 'Ovaj računar već ima ugrađen memorijski modul!' : 'Računar je bez memorijskog modula.'}}"
                            value="{{ $r->id }}"
...
<div class="row dugmici">
    <div class="col-md-6 col-md-offset-6">
        <div class="form-group text-right">
            ...
            <div class="col-md-6">
                <a class="btn btn-danger btn-block ono" href="{{route('memorije.oprema')}}"><i class="fa fa-ban"></i>&emsp;&emsp;Otkaži</a>
            </div>
        </div>
    </div>
</div>
