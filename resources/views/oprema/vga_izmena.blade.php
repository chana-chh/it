...
@section('naziv', 'Oprema | Izmena grafičkog adaptera')
...
<div class="row">
    <div class="col-md-8">
        <h1>
            <img class="slicica_animirana" alt="Izmena grafičkog adaptera"
                 src="{{url('/images/vga.png')}}" style="height:64px;">
            &emsp;Izmena podataka grafičkog adaptera
        </h1>
    </div>
</div>
<hr>
...
<div class="row ceo_dva">
    <div class="col-md-12 boxic">
        <form action="{{ route('vga.oprema.izmena.post', $uredjaj->id) }}" method="POST" data-parsley-validate>
            {{ csrf_field() }}
                        ...
                        <select name="stavka_otpremnice_id" id="stavka_otpremnice_id" class="chosen-select form-control" data-placeholder="otpremnice ..." >
                            ...
                            <optgroup label="{{ $o->dobavljac->naziv }}, {{ $o->broj }} od {{ $o->datum }}">
                                ...
                                <option value="{{ $s->id }}"
                                    ...
                                    {{$s->naziv}} (popunjeno je {{$s->grafickiAdapteri->count() }} od {{$s->kolicina}} {{$s->jedinica_mere}})
                            </option>
                            @endforeach
                        </optgroup>
                        @endforeach
            ...
            <div class="col-md-4">
                <div class="form-group{{ $errors->has('graficki_adapter_model_id') ? ' has-error' : '' }}">
                    <label for="graficki_adapter_model_id">Model grafičkog adaptera:</label>
                    <select name="graficki_adapter_model_id" id="graficki_adapter_model_id" class="chosen-select form-control" data-placeholder="model ..." required>
                        <option value=""></option>
                        @foreach($modeli as $m)
                        <option value="{{ $m->id }}"
                            {{ $m->id == old('graficki_adapter_model_id') ? ' selected' : '' }}
                            {{ $uredjaj->graficki_adapter_model_id == $m->id ? ' selected' : '' }}>
                            {{ $m->proizvodjac->naziv }} sa čipom {{$m->naziv}}, {{$m->kapacitet_memorije}} MB
                    </option>
                    @endforeach
                </select>
        ...
        <div class="col-md-6">
            <div class="form-group{{ $errors->has('racunar_id') ? ' has-error' : '' }}">
                ...
                <select name="racunar_id" id="racunar_id" class="chosen-select form-control" data-placeholder="računar ..." >
                    ...
                    <option data-procesor="{{ $r->grafickiAdapteri()->count() > 0 ? 'Ovaj računar već ima ugrađen grafički adapter!' : 'Računar je bez grafičkog adaptera.'}}"
                            value="{{ $r->id }}"
...
<div class="row dugmici">
    <div class="col-md-6 col-md-offset-6">
        <div class="form-group text-right">
            ...
            <div class="col-md-6">
                <a class="btn btn-danger btn-block ono" href="{{route('vga.oprema')}}"><i class="fa fa-ban"></i>&emsp;&emsp;Otkaži</a>
            </div>
        </div>
    </div>
</div>
