...
@section('naziv', 'Oprema | Izmena podataka za ažuriranja za servere')
...
<div class="row">
    <div class="col-md-8">
        <h1>
            <i class="fa fa-bomb fa-fw text-primary"></i>&emsp;Izmena podataka za ažuriranja za servere 
        </h1>
    </div>
</div>
<hr>
...
<div class="row ceo_dva">
    <div class="col-md-12 boxic">
        <form action="{{ route('serveri.izmena.up.post', $up->id) }}" method="POST" data-parsley-validate>
            {{ csrf_field() }}
...
<div class="row">
        <div class="col-md-3">
                <div class="form-group{{ $errors->has('server_id') ? ' has-error' : '' }}">
                    ...
                    <select name="server_id" id="server_id" class="chosen-select form-control" data-placeholder="server ..." required>
                        ...
                        <option value="{{ $server->id }}" {{ old( 'server_id')==$server->id ? ' selected' : '' }} {{ $up->server_id == $server->id ? ' selected' : '' }}>{{ $server->ime }}.{{$server->domen}}
                    </option>
                    @endforeach
                </select>
    ...
    <div class="col-md-3">
                   <div class="form-group{{ $errors->has('datum') ? ' has-error' : '' }}">
                        ...
                        <input type="text" name="datum" id="datum" class="form-control datepicker" placeholder="dd.mm.yyyy"
                               value="{{ old('datum', $up->datum) }}" required>
                        @if ($errors->has('datum'))
                        <span class="help-block">
    ...
    <div class="col-md-6">
        <div class="form-group{{ $errors->has('opis') ? ' has-error' : '' }}">
            ...
            <textarea name="opis" id="opis" class="form-control">{{ old('opis', $up->opis) }}</textarea>
            @if ($errors->has('opis'))
            <span class="help-block">
