...
@section('naziv', 'Oprema | Izmena skenera')
...
<div class="row">
    <div class="col-md-8">
        <h1>
            <img class="slicica_animirana" alt="Izmena skenera" src="{{url('/images/scanner.png')}}" style="height:64px;"> &emsp;Izmena podataka skenera
        </h1>
    </div>
</div>
<hr>
<div class="row" style="margin-bottom: 16px;">
    <div class="col-md-12">
        <div class="btn-group">
            ...
            <a class="btn btn-primary" href="{{ route('skeneri.oprema') }}" title="Povratak na listu skenera">
...
<div class="row ceo_dva">
    <div class="col-md-12 boxic">
        <form action="{{ route('skeneri.oprema.izmena.post', $uredjaj->id) }}" method="POST" data-parsley-validate>
            {{ csrf_field() }}
                        ...
                        <select name="stavka_otpremnice_id" id="stavka_otpremnice_id" class="chosen-select form-control" data-placeholder="otpremnice ...">
                            ...
                            <optgroup label="{{ $o->dobavljac->naziv }}, {{ $o->broj }} od {{ $o->datum }}">
                                ...
                                <option value="{{ $s->id }}" {{ $s->id == old('stavka_otpremnice_id') ? ' selected' : '' }} {{ $uredjaj->stavka_otpremnice_id
                                    == $s->id ? ' selected' : '' }}> {{$s->naziv}} (popunjeno je {{$s->skeneri->count()
                        ...
                        <select name="stavka_nabavke_id" id="stavka_nabavke_id" class="chosen-select form-control" data-placeholder="nabavke ...">
                            ...
                            <optgroup label="{{ $o->dobavljac->naziv }} od {{ $o->datum }}">
                                ...
                                <option value="{{ $s->id }}" {{ $s->id == old('stavka_nabavke_id') ? ' selected' : '' }} {{ $uredjaj->stavka_nabavke_id == $s->id
                                    ? ' selected' : '' }}> {{$s->naziv}} (popunjeno je {{$s->skeneri->count() }} od {{$s->kolicina}}
                                    {{$s->jedinica_mere}})
                                </option>
                                @endforeach
                            </optgroup>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group{{ $errors->has('skener_model_id') ? ' has-error' : '' }}">
                        <label for="skener_model_id">Model skenera:</label>
                        <select name="skener_model_id" id="skener_model_id" class="chosen-select form-control" data-placeholder="model ..." required>
                            <option value=""></option>
                            @foreach($modeli as $m)
                            <option value="{{ $m->id }}" {{ $m->id == old('skener_model_id') ? ' selected' : '' }} {{ $uredjaj->skener_model_id == $m->id ?
                                ' selected' : '' }}> {{ $m->proizvodjac->naziv }}, {{ $m->naziv }} sa rezolucijom {{$m->rezolucija}}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="form-group{{ $errors->has('racunar_id') ? ' has-error' : '' }}">
                        ...
                        <select name="racunar_id" id="racunar_id" class="chosen-select form-control" data-placeholder="računar ...">
                            ...
                            <option data-procesor="{{ $r->monitori()->count() > 0 ? 'Ovaj računar je već povezan sa skenerom!' : 'Računar je bez skenera.'}}"
            ...
            <div class="row dugmici">
                <div class="col-md-6 col-md-offset-6">
                    <div class="form-group text-right">
                        ...
                        <div class="col-md-6">
                            <a class="btn btn-danger btn-block ono" href="{{route('skeneri.oprema')}}">
