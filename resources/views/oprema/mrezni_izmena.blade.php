...
@section('naziv', 'Oprema | Izmena mrežnog uređaja')
...
<div class="row">
    <div class="col-md-8">
        <h1>
            <img class="slicica_animirana" alt="Izmena mrežnog uređaja" src="{{url('/images/mrezniUredjaji.png')}}" style="height:64px;"> &emsp;Izmena podataka mrežnog uređaja
        </h1>
    </div>
</div>
<hr>
...
<div class="row ceo_dva">
    <div class="col-md-12 boxic">
        <form action="{{ route('mrezni.oprema.izmena.post', $uredjaj->id) }}" method="POST" data-parsley-validate>
            {{ csrf_field() }}
                        ...
                        <select name="stavka_otpremnice_id" id="stavka_otpremnice_id" class="chosen-select form-control" data-placeholder="otpremnice ...">
                            ...
                            <optgroup label="{{ $o->dobavljac->naziv }}, {{ $o->broj }} od {{ $o->datum }}">
                                ...
                                <option value="{{ $s->id }}" {{ $s->id == old('stavka_otpremnice_id') ? ' selected' : '' }} {{ $uredjaj->stavka_otpremnice_id
                                    == $s->id ? ' selected' : '' }}> {{$s->naziv}} (popunjeno je {{$s->mrezniUredjaji->count()
                        ...
                        <select name="stavka_nabavke_id" id="stavka_nabavke_id" class="chosen-select form-control" data-placeholder="nabavke ...">
                            ...
                            <optgroup label="{{ $o->dobavljac->naziv }} od {{ $o->datum }}">
                                ...
                                <option value="{{ $s->id }}" {{ $s->id == old('stavka_nabavke_id') ? ' selected' : '' }} {{ $uredjaj->stavka_nabavke_id == $s->id
                                    ? ' selected' : '' }}> {{$s->naziv}} (popunjeno je {{$s->mrezniUredjaji->count() }} od
                                    {{$s->kolicina}} {{$s->jedinica_mere}})
                                </option>
                                @endforeach
                            </optgroup>
                            @endforeach
            ...
            <div class="row">
                <div class="col-md-2">
                    <div class="form-group{{ $errors->has('naziv') ? ' has-error' : '' }}">
                        ...
                        <input type="text" name="naziv" id="naziv" class="form-control" value="{{ old('naziv', $uredjaj->naziv) }}" maxlength="50"> @if($errors->has('naziv'))
                        <span class="help-block">
                ...
                <div class="col-md-2">
                    <div class="form-group{{ $errors->has('broj_portova') ? ' has-error' : '' }}">
                        ...
                        <input type="text" name="broj_portova" id="broj_portova" class="form-control" value="{{ old('broj_portova', $uredjaj->broj_portova) }}"
                            maxlength="50"> @if($errors->has('broj_portova'))
                        <span class="help-block">
                ...
                <div class="col-md-3">
                    <div class="form-group checkboxoviforme">
                        <label>
                            <input type="checkbox" name="upravljiv" id="upravljiv" {!!$uredjaj->upravljiv == 0 ? "" :'checked="checked"'!!}> &emsp;Da li se radi o upravljivom uređaju?
                        </label>
                    </div>
                </div>
                <div class="col-md-5">
                    <div class="form-group{{ $errors->has('link') ? ' has-error' : '' }}">
                        ...
                        <textarea id="link" name="link" class="form-control">{{ old('link', $uredjaj->link) }}</textarea>
                        @if ($errors->has('link'))
                        <span class="help-block">
            ...
            <div class="row dugmici">
                <div class="col-md-6 col-md-offset-6">
                    <div class="form-group text-right">
                        ...
                        <div class="col-md-6">
                            <a class="btn btn-danger btn-block ono" href="{{route('mrezni.oprema')}}">
