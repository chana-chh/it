...
@section('naslov')
        ...
        <h1 class="page-header"><span><img class="slicica_animirana" alt="Modeli čvrstih diskova - dodavanje" src="{{url('/images/hdd.png')}}" style="height:64px;"></span>&emsp;Dodavanje modela čvrstog diska</h1>
                ...
                <div class="col-md-3">
                   <div class="form-group{{ $errors->has('povezivanje_id') ? ' has-error' : '' }}">
                    ...
                    <select name="povezivanje_id" id="povezivanje_id" class="chosen-select form-control" data-placeholder="tipovi ..." required>
                        ...
                        <option value="{{ $t->id }}"{{ old('povezivanje_id') == $t->id ? ' selected' : '' }}>
                            {{ $t->naziv }}
                        </option>
                        @endforeach
                    </select>
