...
class OsnovnePloceKontroler extends Kontroler
{
    ...
    public function postDodavanje(Request $request)
    {
        ...
        $this->validate($request, [
            'naziv' => [
                'required',
                'unique:osnovne_ploce_modeli,naziv'],
            'cipset' => [
                'required'],
            'proizvodjac_id' => [
                'required'],
    ...
    public function postIzmena(Request $request, $id)
    {
        ...
        $this->validate($request, [
            'naziv' => [
                'required',
                'unique:osnovne_ploce_modeli,naziv,' . $id],
            'cipset' => [
                'required'],
            'proizvodjac_id' => [
                'required'],
    ...
    public function getUredjaji($id)
    {
        ...
        $osnovne_ploce = OsnovnaPloca::with([
                    'racunar',
                    'stavkaOtpremnice.otpremnica'])->where('osnovna_ploca_model_id', '=', $id)->get();
        $model = OsnovnaPlocaModel::find($id);
